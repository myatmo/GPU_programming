...
/*---------- prototype functions   */
  ...
  float calculate_stdev(int *data, int n);
/*---------- begin */
  MPI_Init(&argc,&argv);
...
/*---------- define ntot and n (n=actual size of Loc_dat) */
  // just a test
  //n = 200;
  ntot = 100000;//n*nprocs;
  n = ntot/nprocs;
/*---------- generate some data */
  seed = 1 + myid*2;//163 + myid*211;
  for (j=0; j<n; j++)
    Loc_dat[j] = ((j+1)*157 + 397*seed)  %   278459; //((j+1)*5 + seed);//

  // calculate standard deviation and mean
  stdev = calculate_stdev(Loc_dat, n);
  printf("standard deviation of the data on proc %d: %f\n",myid, stdev);
